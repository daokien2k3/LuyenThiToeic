...
export default function TestDetail() {
    ...
    const handleStartTest = () => {
        ...
        setError("");
        navigate(`/tests/${id}/${mode}/${selectedParts.join(",") || "all"}`);
    };
...
return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-100">
        <div className="max-w-6xl mx-auto px-6 py-12">
            ...
            {/* Chọn phần thi */}
            <div className="bg-gray-50 rounded-2xl p-8 mb-10 shadow-lg ring-1 ring-gray-200">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-gray-800">Chọn phần thi bạn muốn làm</h2>
                    {mode === "practice" && parts.length > 0 && (
                        <button 
                            onClick={handleSelectAllParts} 
                            className="text-sm text-blue-600 hover:text-blue-800 font-semibold underline transition-colors duration-200"
                        >
                ...
                {parts.length === 0 ? (
                    <p className="text-gray-500 text-center py-8 text-lg">Đề thi này chưa có phần thi nào.</p>
                ) : (
                    <div className="space-y-6">
                        {parts.map((part) => (
                            <div key={part.id} className="bg-white p-6 rounded-xl shadow-md border border-gray-200 hover:shadow-lg hover:border-blue-300 transition-all duration-300">
                                <label className="flex items-center gap-3 font-semibold text-blue-700 cursor-pointer mb-3 text-lg">
                                    {mode === "practice" && (
                                        <input
                                            type="checkbox"
                                            ...
                                            className="w-5 h-5 accent-blue-600 rounded focus:ring-2 focus:ring-blue-300"
                                        />
                                    )}
                                    {part.title}
                                    <span className="text-gray-600 text-sm ml-2 bg-gray-100 px-2 py-1 rounded-full">
                                        (
                                        {mediaQuestions
                                ...
                                <div className="flex flex-wrap gap-3 mt-3">
                                    {part.types?.map((type, index) => (
                                        <span key={index} className="bg-gradient-to-r from-gray-100 to-gray-200 text-gray-600 text-xs px-3 py-1 rounded-full border border-gray-300 shadow-sm hover:shadow-md transition-shadow duration-200">
                                            {type}
                                        </span>
                                    ))}
                                </div>
