...
export default function TestDetail() {
    ...
    const [customTime, setCustomTime] = useState(null); // Khởi tạo null để set sau khi có exam
    ...
    useEffect(() => {
        const fetchExam = async () => {
            try {
                ...
                setExam(json.data);
                // Set thời gian mặc định = thời gian tiêu chuẩn của đề thi
                setCustomTime(json.data.TimeExam);
            } catch (err) {
    ...
    const maxTime = exam.TimeExam * 2; // Thời gian tối đa = gấp đôi thời gian tiêu chuẩn
    ...
    const handleStartTest = async () => {
        try {
            ...
            if (customTime < 5 || customTime > maxTime) {
                setError(`Thời gian phải từ 5 đến ${maxTime} phút`);
                return;
            }
