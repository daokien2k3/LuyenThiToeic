...
export default function Statistics() {
    ...
    // Kiểm tra xem có phải FULL_TEST không
    const isFullTest = () => selectedExamType === "FULL_TEST";

    // Lấy max score dựa trên loại đề
    const getMaxScore = () => isFullTest() ? 495 : 100;
    ...
    // Lọc kết quả theo loại đề thi và ngày
    const getFilteredResults = () => {
        let filtered = results;

        // Lọc theo loại đề thi
        if (selectedExamType) {
            filtered = filtered.filter(r => r.examTypeCode === selectedExamType);
        }

        // Lọc theo số ngày
        if (selectedDays !== "all") {
            const daysCount = parseInt(selectedDays);
    ...
    const calculateTabStats = (type) => {
        ...
        return {
            ...
            accuracy: `${Math.round((avgScore / max) * 100)}%`,
            avgTime: `${avgTime} phút`,
            avgScore: `${avgScore}/${max}`,
            bestScore: `${maxScore}/${max}`,
        };
    };
    ...
    const getSelectedExamTypeName = () => {
        ...
        return examType ? examType.Code : "Loại đề";
    };
    ...
    const totalMaxScore = isFullTest() ? 990 : 200;
                    ...
                    {selectedExamType && (
                        <div className="mt-3 flex flex-wrap gap-2 items-center">
                            ...
                            {/* <span className="px-3 py-1 bg-blue-100 text-blue-700 text-xs font-semibold rounded-full">
                                {isFullTest() ? "Thang điểm TOEIC (0-495)" : "Thang điểm phần trăm (0-100)"}
                            </span> */}
                        </div>
                    )}
                </div>
                        ...
                        <ResponsiveContainer width="100%" height={250}>
                            <PieChart>
                                <Pie
                                    ...
                                    cy="50%"
                                    labelLine={false}
                                    label={({ name, value }) => `${name}: ${value}/${maxScore}`}
                                    outerRadius={80}
                                    fill="#8884d8"
                                    dataKey="value"
                                >
                                ...
                                >
                                    ...
                                    <div className="flex flex-wrap gap-6 text-gray-600 text-sm mt-3 bg-gray-50 p-3 rounded-lg">
                                        <span className="flex items-center gap-1">
                                            🎧 Listening: <strong>{r.ScoreListening}/{maxScore}</strong>
                                        </span>
                                        <span className="flex items-center gap-1">
                                            📖 Reading: <strong>{r.ScoreReading}/{maxScore}</strong>
                                        </span>
                                        <span className="flex items-center gap-1">
